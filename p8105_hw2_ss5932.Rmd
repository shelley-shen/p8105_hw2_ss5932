...
- There are `r filter(nyc_transit_tidy, route_number == "A") %>% filter(ada == "TRUE") %>% distinct(line, station_name) %>% count()` distinct stations that serve the A train and are ADA compliant. 
...
Read and clean data in unemployment csv file using similar process as above and using pivot_longer to merge the month variables into one columnn and their respective values into a separate percentage unemployed column. I will also convert year and month into factor variables to ensure that it can merge with the other datasets later. 
...
The snp dataset contains information regarding the Standard & Poor's stock market index (S&P) and the closing values of the S&P on the associated date. NA values arise in this dataset on dates prior to 1950, indicating that no measurements were obtained or available of the closing values of the S&P prior to January 1950. There are a total of `r nrow(snp_df)` observations in the snp dataset which ranges from 1950 until 2015. 

The unemployment dataset contains information regarding the percentage of unemployment on the associated date. NA values arise in this dataset on dates prior to 1948, indicating that no measurements were obtained or available of unemployment until January 1948. There are a total of `r nrow(unemploy_df)` observations in the unemployment dataset which ranges from 1948 until 2015. 
